...
**It never takes input.** The window is `WS_EX_TRANSPARENT | WS_EX_NOACTIVATE` and
answers `WM_NCHITTEST` with `HTTRANSPARENT`. Clicks, keystrokes and Windows Hello
prompts all reach the credential UI as if it were not there. A logon screen that eats
input is a machine its owner cannot get into.
